...
//my semaphore
SEMAPHORE *sem_readwrite_mutex;
SEMAPHORE *sem_flush_ready;
SEMAPHORE *sem_writebuffer_free;
SEMAPHORE *sem_isRead;
...
void UserMain(int argc, char *argv[])
{
  ...
  //my sem
  sem_readwrite_mutex =  NewSemaphore("memreq",1);  //Used to lock read and write, make sure they are mutex
  sem_flush_ready =  NewSemaphore("memreq",0);  //Used to signal writer that flush is read
  sem_writebuffer_free =  NewSemaphore("sem_enqueue", MAX_QUEUE_SIZE);  //Used to signal flush that write buffer is free
